...
var MuroCaja = preload("res://tiles/Cosas nabil uwu/muro_caja.tscn")
var color = -1
var move_inputx = 0;
var move_inputy = 0;
var is_in_area = 0;
...
func _ready():
	...
	var object = MuroCaja.instantiate()
	add_child(object)
...
func _process(delta):
	if color != ColorGlobal.ColorGlobal:
		if ColorGlobal.ColorGlobal == 2:
			if find_child("MuroCaja",true,false) == null:
				var object = MuroCaja.instantiate()
				add_child(object)
			color = ColorGlobal.ColorGlobal
		
		if ColorGlobal.ColorGlobal == 0 or ColorGlobal.ColorGlobal == 1:
			if find_child("MuroCaja",true,false) != null:
				for nodo in find_children("MuroCaja","Object",true,false):
					nodo.queue_free()
			color = ColorGlobal.ColorGlobal
