...
var MuroCajaV = preload("res://tiles/Cosas nabil uwu/muro_caja_v.tscn")
var color = -1
var move_inputx = 0;
var move_inputy = 0;
var is_in_area = 0;
...
func _ready():
	...
	var object = MuroCajaV.instantiate()
	add_child(object)
...
func _process(delta):
	if color != ColorGlobal.ColorGlobal:
		if ColorGlobal.ColorGlobal == 0:
			if find_child("MuroCajaV",true,false) == null:
				var object = MuroCajaV.instantiate()
				add_child(object)
			color = ColorGlobal.ColorGlobal
		...
		if ColorGlobal.ColorGlobal == 1 or ColorGlobal.ColorGlobal == 2:
			if find_child("MuroCajaV",true,false) != null:
				for nodo in find_children("MuroCajaV","Object",true,false):
					nodo.queue_free()
			color = ColorGlobal.ColorGlobal
